...
from utils_attach import attach_to, landmarks_BB
import torch
import warnings
warnings.filterwarnings('ignore')
from data import DataStorage, BaostockDataWorker, TushareDataWorker 
...
class Preprocessor():
    ...
    def clean(self,df:pd.DataFrame = None):
        if isinstance(self.dw, BaostockDataWorker):
            return self.__clean_baostock__(df)
        elif isinstance(self.dw, TushareDataWorker):
            return self.__clean_tushare__(df)
        else:
            raise NotImplementedError("only support BaostockDataWorker and TushareDataWorker")
    ...
    def __clean_baostock__(self,df:pd.DataFrame = None):
...
if __name__ == "__main__":
    df = pd.read_csv('E://trading-data.20230804//index_data//sz399015.csv')
    final = Preprocessor().load(df).bundle_process()
