...
from EncoderDecoderAgent.iTransformer.Seq2SeqModel import Seq2Seq
from EncoderDecoderAgent.iTransformer.Encoder import Encoder
from EncoderDecoderAgent.iTransformer.Decoder import Decoder
...
class Train(BaseTrain):
    def __init__(self,
        ...
        self.policy_decoder = Decoder(self.data_train.state_size)
        self.target_decoder = Decoder(self.data_train.state_size)
        ...
        # # 仅encoder，把原来的decoder先去掉
        # self.policy_net = self.encoder.to(device)
        # self.target_net = self.encoder.to(device)
        ...
        test_encoder = Encoder(self.config).to(device)
        test_decoder =Decoder(self.data_train.state_size).to(device)

        self.test_net = Seq2Seq(test_encoder, test_decoder)
        # self.test_net = test_encoder.to(device)
