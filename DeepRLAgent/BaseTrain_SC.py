...
from DeepRLAgent.losses import SupConLoss
from DeepRLAgent.Supcon_net import Supcon_net
...
class BaseTrain:
    def __init__(self,
        ...
        self.memory = ReplayMemory(ReplayMemorySize, mode='SC')
        self.Transition = self.memory.Transition

        self.Supcon_net = Supcon_net(data_train.state_size).to(device)
        self.optimizer_Supcon = optim.Adam(self.Supcon_net.parameters())
        # without data augumentation
        self.supcon_loss = SupConLoss(contrast_mode='one')
    ...
    def select_action(self, state):
        ...
        if sample > eps_threshold:
            with torch.no_grad():
                ...
                self.Supcon_net.eval()
                state = self.Supcon_net(state.unsqueeze(1)).squeeze(1)
    ...
    def optimize_model(self):
        ...
        label_batch = torch.cat(batch.label)
        next_label_batch = torch.cat([l for l in batch.next_label if l is not None])

        # Use supervised contrastive model to convert state
        # label_batch:[BATCH_SIZE,1]
        # state_batch:[BATCH_SIZE,windowsize*4]
        # non_final_next_states:[BATCH_SIZE,windowsizef*4]
        # ↓ should transform to ↓
        # label_batch:[BATCH_SIZE]
        # state_batch:[BATCH_SIZE,1,windowsize*4] 
        # non_final_next_states:[BATCH_SIZE,1,windowsizef*4]

        labels = torch.cat([label_batch,next_label_batch])
        
        merge_state = torch.cat([state_batch.unsqueeze(1),non_final_next_states.unsqueeze(1)],dim=0)

        merge_state = self.Supcon_net(merge_state)
    
        loss_supcon = self.supcon_loss(merge_state, labels)
        self.optimizer_Supcon.zero_grad()
        loss_supcon.backward()
        # for param in self.Supcon_net.parameters():
        #     param.grad.data.clamp_(-1, 1)
        self.optimizer_Supcon.step()
        ...
        # with torch.no_grad():
        #     state_batch = self.Supcon_net(state_batch.unsqueeze(1)).squeeze(1)
        #     non_final_next_states = self.Supcon_net(non_final_next_states.unsqueeze(1)).squeeze(1)
    ...
    def train(self, num_episodes=50):
        ...
        for i_episode in tqdm(range(num_episodes)):
            ...
            for t in count():
                ...
                done, reward, next_state, label, next_label = self.data_train.step(action.item())

                reward = torch.tensor([reward], dtype=torch.float, device=device)
                label = torch.tensor([label], dtype=torch.int, device=device)
                if next_label is not None:
                    next_label = torch.tensor([next_label], dtype=torch.int, device=device)

                if next_state is not None:
                    next_state = torch.tensor([next_state], dtype=torch.float, device=device)

                # Store the transition in memory
                self.memory.push(done, state, action, next_state, reward,label,next_label)
